...
def build_cases(entries, max_targets=0, max_exogs=0, suite="ultra", max_pairs=0, max_trios=0):
    ...
    for target in _choose_targets(entries, max_targets):
        ...
        # Baselines
        cases.append({"indicator": target, "exog": [], "profile": "default", "max_lag": lag_profiles["lag6"]})
        cases.append({"indicator": target, "exog": [], "profile": "conservador", "max_lag": lag_profiles["lag3"]})

        # Individuales
        for ex in exogs:
            cases.append({"indicator": target, "exog": [ex], "profile": "default", "max_lag": lag_profiles["lag6"]})

        # Pares y trios
        pairs = _slice_for_cap(combinations(exogs, 2), max_pairs)
        for pair in pairs:
            cases.append({"indicator": target, "exog": list(pair), "profile": "default", "max_lag": lag_profiles["lag6"]})

        trios = _slice_for_cap(combinations(exogs, 3), max_trios)
        for trio in trios:
            cases.append({"indicator": target, "exog": list(trio), "profile": "default", "max_lag": lag_profiles["lag6"]})

        if exogs:
            cases.append({"indicator": target, "exog": exogs, "profile": "default", "max_lag": lag_profiles["lag3"]})
...
def main():
    parser = argparse.ArgumentParser(description="Stress test XGBoost con datos reales de BD")
