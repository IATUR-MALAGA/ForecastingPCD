...
def build_cases(
    ...
    for target in _choose_targets(entries, max_targets):
        ...
        for pair in pairs:
            cases.append(
                {"indicator": target, "exog": list(pair), "profile": "default"}
            )

        trios = _slice_for_cap(combinations(exogs, 3), max_trios)
        for trio in trios:
            cases.append(
                {"indicator": target, "exog": list(trio), "profile": "default"}
            )
        ...
        if suite == "ultra":
            ...
            for aset in anchor_sets:
                ...
                for profile in ["trend", "seasonal_light"]:
                    cases.append(
                        {"indicator": target, "exog": aset, "profile": profile}
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Stress test SARIMAX con datos reales de BD"
    )
